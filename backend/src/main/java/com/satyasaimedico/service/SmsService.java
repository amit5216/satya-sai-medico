...
public class SmsService {
    ...
    public void init() {
        if (accountSid != null && !accountSid.isBlank() &&
            ...
            log.warn("Twilio credentials not configured — SMS features disabled");
        }
    }
    ...
    /**
     * Send OTP for mobile verification during appointment booking.
     */
    public void sendOtp(String mobile, String otp) {
        String message = String.format(
            "Your Satya Sai Medico verification code is: %s. " +
            "Valid for 5 minutes. Do not share this code. -Satya Sai Medico",
            otp
        );
        sendSms(mobile, message);
    }
    ...
    private void sendSms(String mobile, String message) {
        if (!twilioEnabled) {
            log.info("SMS skipped (Twilio disabled) for {}: {}", mobile, message.substring(0, Math.min(50, message.length())));
            return;
        }
        try {
